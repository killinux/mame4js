...
std::string running_machine::compose_saveload_filename(std::string &&filename, const char **searchpath)
{
	...
	// is this an absolute path? ,haohao osd_is_absolute_path
    ...
    osd_printf_info("haohao------->compose_saveload_filename: [%s]\n",result);// [tengai/q.sta]
	return result;
}
...
{
	// compose the save/load filename and persist it ,haohao
...
void running_machine::handle_saveload() //haohao ,maybe save_state is here
{
	// if no name, bail
